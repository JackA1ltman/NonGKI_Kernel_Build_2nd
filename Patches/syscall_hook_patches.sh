...
for i in "${patch_files[@]}"; do
    ...
    if grep -Eq "ksu_handle|ksu_legacy_" "$i"; then
        echo "[-] Warning: $i contains KernelSU"
        echo "[+] Code in here:"
        grep -En "ksu_handle|ksu_legacy_" "$i"
        echo "[-] End of file."
        echo "======================================"
        continue
    fi
    ...
    fs/exec.c)
        ...
        if grep -rq --include="*.c" --include="*.h" "ksu_legacy_execve_sucompat" "drivers/kernelsu/" >/dev/null 2>&1 \
            && grep -q '^static int do_execve_common(const char \*filename,' fs/exec.c \
            && grep -q 'if ((current->flags & PF_NPROC_EXCEEDED)' fs/exec.c; then
            sed -i '/^static int do_execve_common(const char \*filename,/i\#ifdef CONFIG_KSU\n__attribute__((hot))\nextern int ksu_legacy_execve_sucompat(const char **filename_ptr,\n\t\t\t\t      void *argv, void *envp);\n#endif\n' fs/exec.c
            sed -i '/if ((current->flags & PF_NPROC_EXCEEDED)/i\#ifdef CONFIG_KSU\n\tksu_legacy_execve_sucompat(\&filename, (void *)\&argv, (void *)\&envp);\n#endif\n' fs/exec.c
            EXEC_HOOK="ksu_legacy_execve_sucompat"
        elif grep -rq --include="*.c" --include="*.h" "ksu_handle_execveat" "drivers/kernelsu/" >/dev/null 2>&1 \
            && grep -q 'int do_execve(struct filename \*filename,' fs/exec.c \
            && grep -q 'return do_execveat_common(AT_FDCWD, filename, argv, envp, 0);' fs/exec.c; then
            ...
            EXEC_HOOK="ksu_handle_execveat"
        else
            EXEC_HOOK=""
            echo "[-] fs/exec.c layout or KernelSU exec hook API is unsupported, skipped."
        fi

        if [ -n "$EXEC_HOOK" ] && [ "$(grep -c "$EXEC_HOOK" fs/exec.c)" -ge 2 ]; then
            echo "[+] fs/exec.c Patched!"
            echo "[+] Count: $(grep -c "$EXEC_HOOK" fs/exec.c)"
        elif [ -n "$EXEC_HOOK" ]; then
    ...
    security/selinux/hooks.c)
        if grep -q "security_secid_to_secctx" "security/selinux/hooks.c" >/dev/null 2>&1; then
        ...
        elif [ "$FIRST_VERSION" -lt 5 ] && [ "$SECOND_VERSION" -lt 10 ] \
            && grep -q 'int nnp = (bprm->unsafe & LSM_UNSAFE_NO_NEW_PRIVS);' security/selinux/hooks.c \
            && grep -q 'if (!nnp && !nosuid)' security/selinux/hooks.c \
            && grep -q 'return 0; /\* No change in credentials \*/' security/selinux/hooks.c; then
            ...
            echo "[-] SELinux credential layout does not need or support this fix, skipped."
        fi
